...
Added a login page to `scroller-front-end-poc` mirroring the authentication pattern from `finder-evaluation-dashboard`. The implementation includes hardcoded users (jack and phil with bcrypt-hashed passwords), JWT cookie-based authentication, an `AuthContext` React context, a protected route group that verifies the auth cookie on the server, and a matching login page UI. Authentication now requires `JWT_SECRET_KEY` to be configured before login or session verification routes can run, rather than falling back to a shared default secret.
...
- `src/app/(protected)/layout.tsx`: Server-side guard for authenticated pages that redirects to `/login` before protected page content is rendered when the auth cookie is missing or invalid
- `src/components/ProtectedRoute.tsx`: Client component inside the protected route group that redirects unauthenticated users away from protected routes during client navigation
...
- `src/app/(protected)/page.tsx`: Preserves `/` inside the protected route group so the home page stays behind authentication
- `src/app/layout.tsx`: Wraps the app with `<AuthProvider>`
