 ...
 * header here, on the server, so it is never exposed to the browser. Network
 * failures surface as a status-0 error and any non-2xx upstream response throws
 * with the upstream status so the BFF route can map it to a stable FE response.
 */
